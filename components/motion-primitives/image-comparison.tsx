...
/** Owner edit: expose the slider position so arbitrary content (SVG, video) can be clipped, not just <img>. */
function useImageComparison() {
  const ctx = useContext(ImageComparisonContext);
  if (!ctx) throw new Error('useImageComparison must be used inside <ImageComparison>');
  return ctx;
}

/** Owner edit: clip any children to one side of the slider. */
const ImageComparisonLayer = ({
  ...
  const clipPath = useTransform(motionSliderPosition, (value) =>
    position === 'left' ? `inset(0 0 0 ${value}%)` : `inset(0 ${100 - value}% 0 0)`
  );
  return (
    <motion.div className={cn('absolute inset-0 h-full w-full', className)} style={{ clipPath }}>
      {children}
    </motion.div>
  );
};
...
export { ImageComparison, ImageComparisonImage, ImageComparisonSlider, ImageComparisonLayer, useImageComparison };
